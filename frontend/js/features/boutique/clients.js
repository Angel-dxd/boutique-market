...
export const renderClients = async (container) => {
    ...
    // --- FUNCIÓN ASÍNCRONA PARA MYSQL CON MANEJO DE ERRORES ROBUSTO ---
    const refreshData = async () => {
        try {
            ...
            loadError = "No se pudo conectar con el servidor. Verifica tu conexión.";
            safeRender();
        }
    };
    ...
    const safeRender = () => {
            ...
            <div class="space-y-6 h-full flex flex-col min-h-[600px] animate-fade-in-up">
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between gap-4 items-center">
                    <div>
                        ...
                        <p class="text-gray-500 mt-1">${loadError ? 'Error en la carga' : clients.length + ' clientes registrados'}</p>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3 w-full md:w-auto">
                        <div class="relative flex-1 md:w-64">
                         ...
                         <button
                            id="importBtn"
                            class="bg-blue-50 text-blue-600 px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-blue-100 transition-colors flex items-center gap-2 border border-blue-100 ${loadError ? 'opacity-50 cursor-not-allowed' : ''}"
                            ${loadError ? 'disabled' : ''}
                        >
                        ...
                        <button
                            id="newClientBtn"
                            class="bg-emerald-600 text-white px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-600/20 flex items-center gap-2 ${loadError ? 'opacity-50 cursor-not-allowed' : ''}"
                            ${loadError ? 'disabled' : ''}
                        >
                        ...
                        ${currentUser === 'santi' || true ? `
                            <button
                                id="exportBtn"
                                class="bg-gray-100 text-gray-600 px-4 py-2.5 rounded-xl text-sm font-bold hover:bg-gray-200 transition-colors flex items-center gap-2 ${loadError ? 'opacity-50 cursor-not-allowed' : ''}"
                                ${loadError ? 'disabled' : ''}
                            >
        ...
        if (loadError) {
            htmlContent += `
                <div class="bg-red-50 border border-red-200 rounded-2xl flex-1 flex flex-col items-center justify-center p-8 text-center shadow-sm">
                    <div class="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mb-4">
                        <i data-lucide="wifi-off" width="32" class="text-red-500"></i>
                    </div>
                    <h3 class="text-xl font-bold text-red-700 mb-2">Problema de Conexión</h3>
                    <p class="text-red-600 mb-6 max-w-md">${loadError}</p>
                    <button id="retryBtn" class="bg-red-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-red-600/20 hover:bg-red-700 transition-all flex items-center gap-2">
                        <i data-lucide="refresh-cw" width="18"></i> Reintentar Conexión
                    </button>
                </div>
            `;
        } else {
            ...
            htmlContent += `<div class="bg-white rounded-2xl shadow-sm border border-gray-100 flex-1 overflow-hidden flex flex-col">`;
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
                    ...
                    <form id="clientForm" class="p-6 space-y-5">
                        ...
                        <div class="pt-4 flex gap-3">
                            ...
                            <button type="submit" class="flex-1 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold shadow-lg shadow-slate-900/20 transition-all flex justify-center items-center gap-2">
                                Guardar
                            </button>
                        </div>
                    </form>
        ...
        if (importBtn && importInput) {
            ...
            importInput.onchange = async (e) => {
                ...
                reader.onload = async (event) => {
                    ...
                    api.hideLoading();
                    api.showToast(`Se importaron ${count} clientes correctamente.`, false);
                    await refreshData();
                };
